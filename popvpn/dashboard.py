...
<title>POPVPN X — داشبورد زنده</title>
<meta name="description" content="آمار زنده و کنترل کیفیت خروجی‌های POPVPN X"/>
<style>
  :root{
...
<div class="wrap">
  ...
  <div class="foot">POPVPN X — خروجی خط لولهٔ به‌روزرسانی خودکار · هیچ credentialی در این صفحه ذخیره نمی‌شود</div>
</div>
...
function renderCards(d){
  const probe = d.probe || {}, audit = d.audit || {}, cache = d.cache || {}, quality = d.quality || {};
  $("cards").innerHTML = [
    card("کانفیگ‌های منتشرشده", (d.total||0).toLocaleString(), `${(d.duration_ms/1000).toFixed(1)} ثانیه`),
    card("تکراری حذف‌شده", (d.duplicates||0).toLocaleString(), `${(d.invalid||0).toLocaleString()} خط نامعتبر`),
    card("کشورها", d.country_count || 0, `${(d.by_country||[]).length} کشور در خروجی`),
    card("منابع فعال", (d.sources||[]).filter(s=>s.last_configs>0).length, `${(d.sources||[]).length} منبع`),
    card("تأییدشده با TCP", (probe.alive||0).toLocaleString(), `${(probe.dead||0).toLocaleString()} ناموفق · ${(probe.endpoints_unprobed||0).toLocaleString()} تست‌نشده`),
    card("فیلتر کیفیت", (quality.published||d.total||0).toLocaleString(), `${(quality.excluded_unverified||0).toLocaleString()} تأییدنشده حذف شد`),
    card("نرخ کش HTTP", Math.round((cache.hit_rate||0)*100)+"%", `${cache.hits||0} پاسخ از کش`),
    card("فیلتر امنیتی", (audit.dropped||0).toLocaleString(), `${(audit.private_hosts||0)} میزبان خصوصی`),
  ].join("");
}
